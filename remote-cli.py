...
@click.option(
    '--api-key',
    envvar=['DASHBOARD_API_KEY', 'API_ACCESS_TOKEN'],
    default=None,
    help='Same secret as server API_ACCESS_TOKEN (header X-API-Key). '
         'Env: DASHBOARD_API_KEY or API_ACCESS_TOKEN.',
)
@click.pass_context
def cli(ctx, url, api_key):
    ...
    When the dashboard sets environment variable ``API_ACCESS_TOKEN``, all
    ``/api/*`` routes except ``GET /api/health`` require authentication.
    Pass the same value via ``--api-key`` or ``DASHBOARD_API_KEY`` / ``API_ACCESS_TOKEN``.

    Examples:

        # Query local container (no token on server)
        remote-cli.py dashboard

        # Query remote dashboard
        remote-cli.py --url http://dashboard.example.com:5000 dashboard

        # Server uses API_ACCESS_TOKEN — use the same value on the client:
        export DASHBOARD_API_KEY=your-shared-secret
        remote-cli.py --url http://dashboard.example.com:5000 dashboard

        # Or pass on the command line
        remote-cli.py --api-key YOUR_SHARED_SECRET dashboard
    """
...
def make_request(ctx, endpoint, method='GET'):
    ...
    try:
        ...
        click.echo("   Stellen Sie sicher, dass das Dashboard läuft und erreichbar ist.", err=True)
        sys.exit(1)
        ...
        if e.response.status_code == 401:
            click.echo("✗ Fehler: Authentifizierung fehlgeschlagen (HTTP 401).", err=True)
            click.echo(
                "   Der Server nutzt vermutlich API_ACCESS_TOKEN: denselben Wert als "
                "--api-key übergeben oder DASHBOARD_API_KEY / API_ACCESS_TOKEN setzen "
                "(Header X-API-Key; alternativ Authorization: Bearer …).",
                err=True,
            )
...
@click.pass_context
def version(ctx):
    """Show API liveness (/api/health) and optionally verify /api/systems."""
    base = ctx.obj['url'].rstrip('/')
    try:
        r = requests.get(f"{base}/api/health", timeout=15)
        r.raise_for_status()
        payload = r.json()
    except requests.exceptions.RequestException as exc:
        click.echo(f"✗ API Liveness fehlgeschlagen ({base}): {exc}", err=True)
        sys.exit(1)

    click.echo(f"✓ Verbunden mit: {base}")
    click.echo(f"  API Liveness: {payload.get('status', 'ok')} ({payload.get('service', '')})")

    if ctx.obj['headers']:
        try:
            make_request(ctx, '/api/systems')
            click.echo("  Geschützte Endpunkte (/api/systems): OK (API-Schlüssel akzeptiert)")
        except SystemExit:
            pass
        return

    try:
        probe = requests.get(f"{base}/api/systems", timeout=15)
        if probe.status_code == 401:
            click.echo(
                "  Hinweis: /api/systems ist geschützt — setzen Sie denselben Wert wie "
                "Server-API_ACCESS_TOKEN (z. B. export DASHBOARD_API_KEY=…)."
            )
            return
        probe.raise_for_status()
        click.echo("  Geschützte Endpunkte: ohne zusätzlichen Schlüssel erreichbar")
    except requests.exceptions.RequestException as exc:
        click.echo(f"  Warnung: /api/systems nicht lesbar: {exc}", err=True)
